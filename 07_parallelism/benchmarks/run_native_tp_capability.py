"""Probe official DeviceMesh Colwise/Rowwise Tensor Parallel in CPU/Gloo."""
...
def main() -> None:
    ...
    job = launch_torchrun(
        ...
        rank_directory=args.raw_directory / "native-tp-cpu-probe",
        worker_args=[
            "--mode",
            "native_tp_probe",
            "--backend",
    ...
    payload = build_artifact(
        artifact_type="module07.native_tp_cpu_capability",
        repository_root=REPOSITORY_ROOT,
        environment={
        ...
        config=config,
        measurement={"probe": "DeviceMesh + ColwiseParallel + RowwiseParallel one-step"},
        ...
        boundary="CPU TP proves API semantics, not CUDA/NCCL throughput.",
    )
