# 01 准备工作：仓库基建中的每个文件解决什么问题

这部分不训练模型。它是 01 的前置准备，用于建立可复现、可测试、可协作的项目边界，让后面的实验结果值得相信。
...
| `01_pytorch_training/trainscale_training/` | 01 可安装 Python 包与源码 | 模块代码和学习入口会分离 |
...
本地 Git 已初始化；GitHub 仓库和远端 CI 需要首次创建、push 后才能完成外部验收。这部分不负责模型效果、GPU kernel 或分布式训练。
