...
卷积利用图像的局部空间结构；BatchNorm 在 train mode 使用当前 batch 统计量并更新 running state，在 eval mode 使用保存的统计量；数据增强改变输入分布以降低简单记忆。与 synthetic 规则不同，CIFAR-10 包含真实类内变化，因此 loss 下降和 validation accuracy 高于 10% 随机猜测只能证明模型开始学习，不能代表训练充分。

小子集与短 epoch 能快速暴露数据、shape、mode 和优化链路错误，但样本覆盖不足且 scheduler 很快衰减，预期精度显著低于完整训练。
...
本实验验证真实 CNN 链路而非 CIFAR-10 最终能力。一般情况下，增加数据覆盖和训练长度会继续改善泛化，直到模型容量、增强、优化器或过拟合成为新瓶颈；04 补充实验的完整 50,000/10,000 样本运行把最终验证准确率提高到约 64%–65%，支持但不“证明无限训练必然更好”。
