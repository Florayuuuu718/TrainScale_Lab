"""Benchmark FP32/AMP and eager/compile combinations on a representative workload."""
...
def main() -> None:
    ...
    if base.device != "cuda":
        raise SystemExit("the training-mode benchmark requires a CUDA config")

    variants = [
        ("fp32_eager", "fp32", False),
        ("amp_eager", "amp", False),
        ("fp32_compile", "fp32", True),
        ("amp_compile", "amp", True),
    ]
    ...
    for name, precision, compile_model in variants:
        ...
        if compile_model:
            ...
            os.environ["TORCHINDUCTOR_CACHE_DIR"] = tempfile.mkdtemp(
                prefix=f"cold_{name}_", dir=cache_parent
            )
        ...
        except Exception as error:  # preserve successful controls when one mode fails
            results.append(
                {
                    "variant": name,
                    "status": "failed",
