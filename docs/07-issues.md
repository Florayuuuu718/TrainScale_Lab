...
| 07-03 | 最小 FSDP2 correctness | 真实 `fully_shard`、DTensor placement、reference 一步更新 | 已完成（CUDA/NCCL 2/4 GPU） |
| 07-04 | FSDP2 wrap/shard 实验 | root/layer wrap 的吞吐、峰值显存与 trace | 已完成 |
| 07-05 | 可信 OOM→可运行实验 | 配置驱动的显存转折；不能形成时为 unavailable | unavailable：当前 preset 不自然 OOM，不阻塞 |
| 07-06 | distributed checkpoint/resume | DCP 分片文件及恢复后下一步一致 | 已完成（CUDA/NCCL） |
| 07-07 | 最小 TP correctness | 自定义与原生 Colwise/Rowwise；2/4 rank shape/placement | 已完成（CUDA/NCCL 2/4 GPU） |
| 07-08 | attention/MLP TP 实验 | 输出/梯度/更新；吞吐、显存和 AllReduce trace | 已完成 |
| 07-09 | 并行策略选择树 | DDP/FSDP2/TP 的瓶颈、代价和选择条件 | 已完成 |
| 07-10 | 2D TP×FSDP/DP | 仅在 07A/07B 暴露明确需求后开启 | 未启用（可选） |
| 07-11 | 模块发布验收 | 本地 gates、GPU artifacts、SHA-256、全仓测试 | 已完成 |
...
- PyTorch 2.8 CPU/Gloo FSDP2 探针为 unavailable；CUDA/NCCL preflight 通过。两者按后端分别记录。
